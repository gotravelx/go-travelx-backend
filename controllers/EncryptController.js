...
export const prepareFlightDataForBlockchain = (flightData, encryptionKey) => {
  ...
  if (
    ...
    customLogger.error(
      `Scheduled departure date: ${flightData.scheduledDepartureDate}`
    );
  ...
  if (encryptionKey) {
    ...
    try {
      ...
      const encryptedUtcTimes = selectiveEncrypt(
        blockchainUtcTimes,
        encryptionKey
      );
...
export const decryptString = (encryptedStr, encryptionKey) => {
  // customLogger.info(
  //   "Starting decryptString function",
  //   JSON.stringify({ encryptedStr })
  // );

  try {
    if (!encryptedStr || !encryptedStr.includes(":")) {
      // customLogger.warn(
      //   "String doesn't appear to be encrypted, returning as-is"
      // );
      return encryptedStr;
    }
